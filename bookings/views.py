...
def request_appointment(request):
    if request.method == "POST":
        ...
        return render(
            request, "appointment_requested.html", {"appointment": appointment}
        )
...
def define_time_slots(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "define_time_slots"
            )  # Replace with your URL name for listing time slots
    else:
        form = TimeSlotForm()
...
def send_appointment_notification(appointment):
    ...
    message = (
        f"Your appointment has been requested. Description: {appointment.description}"
    )
...
def add_to_google_calendar(appointment):
    ...
    event = {
        ...
        "end": {
            "dateTime": (
                appointment.requested_time + datetime.timedelta(hours=1)
...
def request_appointment(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            appointment.user = (
                request.user
            )  # Assuming appointment has a 'user' field for the requesting user
            appointment.save()
...
def create_appointment(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            appointment.user = (
                request.user
            )  # Assuming request.user is authenticated and assigned
            appointment.save()
            return redirect(
                "appointment_detail", pk=appointment.pk
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            send_feedback_notification(
                feedback.email
            )  # Call notification function with email
...
def disorder_list(request):
    disorders = [
        ...
        {
            "name": "Mood Disorders (e.g., Depression, Bipolar Disorder)",
            "url": "https://www.mayoclinic.org/diseases-conditions/bipolar-disorder/symptoms-causes/syc-20355955#:~:text=Bipolar%20disorder%2C%20formerly%20called%20manic,or%20pleasure%20in%20most%20activities.",
        },
        {
            "name": "Psychotic Disorders (e.g., Schizophrenia)",
            "url": "https://www.ncbi.nlm.nih.gov/books/NBK546579/#:~:text=A%20psychotic%20episode%20or%20disorder,around%2050%20in%2010000%20people.",
        },
        {
        ...
        },
        {
            ...
            "url": "https://www.mayoclinic.org/diseases-conditions/obsessive-compulsive-disorder/symptoms-causes/syc-20354432#:~:text=Overview,cause%20a%20lot%20of%20distress.",
        },
        {
            "name": "Personality Disorders",
            "url": "https://www.mayoclinic.org/diseases-conditions/personality-disorders/symptoms-causes/syc-20354463#:~:text=A%20personality%20disorder%20is%20a,understanding%20emotions%20and%20tolerating%20distress.",
        },
        {
            "name": "Trauma- and Stressor-Related Disorders (e.g., PTSD)",
            "url": "https://www.msdmanuals.com/home/mental-health-disorders/anxiety-and-stressor-related-disorders/overview-of-trauma-and-stressor-related-disorders",
        },
    ]

    return render(request, "bookings/disorder_list.html", {"disorders": disorders})
